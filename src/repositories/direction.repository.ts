...
export class DirectionRepository {
  async loadFloorConnectors(nodes: Map<string, GraphNode>, edges: GraphEdge[]): Promise<void> {
    const fileNames = await fs.readdir(FLOORS_DIR);

    await Promise.all(
      fileNames.map(async (fileName) => {
        const raw = JSON.parse(await fs.readFile(path.join(FLOORS_DIR, fileName), "utf8"));
  ...
  async loadLocationLinks(nodes: Map<string, GraphNode>, edges: GraphEdge[]): Promise<void> {
    const raw = JSON.parse(await fs.readFile(LINKS_PATH, "utf-8"));
    const parsed = LocationLinksSchema.safeParse(raw);

    if (!parsed.success) {
      logger.error(`[Graph] locationLinks.json is invalid:`, z.treeifyError(parsed.error));
      return;
    }
