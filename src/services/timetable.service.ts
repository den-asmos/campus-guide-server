...
export class TimetableService {
  ...
  async getGroupTimetable(dto: GroupTimetableDto): Promise<DaySchedule[]> {
    ...
    return days.map(({ date, dayName, groups }) => {
      const group = groups.find((g) => g.course === dto.course && g.groupName === dto.group);

      return { date, dayName, groups: group ? [group] : [] };
    });
  }

  async getLecturerTimetable(dto: LecturerTimetableDto): Promise<DaySchedule[]> {
  ...
  async getClassroomTimetable(classroom: string): Promise<DaySchedule[]> {
    ...
    return days.map((day) => ({
      ...day,
      groups: this.filter.filterClassroomGroups(day.groups, classroom.toLowerCase()),
    }));
  }
  ...
  private parseTimetableFiles(downloads: TimetableDownload): DaySchedule[] {
    const days = downloads.filePaths.flatMap((fp) => this.parser.parseTimetable(fp));
  ...
  private formatLecturerName({ firstName, lastName, middleName }: LecturerTimetableDto): string {
    return `${lastName[0].toUpperCase()}${lastName.slice(1)} ${firstName.charAt(0).toUpperCase()}. ${middleName.charAt(0).toUpperCase()}.`;
  }
}
