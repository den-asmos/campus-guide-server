    ...
    return new Promise<UploadApiResponse>((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream(
        ...
        (error: UploadApiErrorResponse | undefined, result: UploadApiResponse | undefined) => {
          if (error) {
            return reject(new AppError(error.message, error.http_code, "CLOUDINARY_UPLOAD_ERROR"));
          }
          if (!result) {
            return reject(new AppError("Нет ответа от cloudinary", 500, "CLOUDINARY_UPLOAD_ERROR"));
          }
          resolve(result);
        },
      );
